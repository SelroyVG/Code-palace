// Задание:
// Написать программу с использованием MPI моделирования процесса изменения смещения точек мембраны при 
// 0 ≤ y ≤ ymax с шагом tau = 0.001, используя геометрический вид параллелизма. Коэффициент c = 1. 
// Мембрана закреплена по краям контура 0 ≤ x ≤ xmax, 0 ≤ y ≤ ymax. 
// В начальный момент временни в точке с координатами (x*, y*) производится удар молоточком 
// перпендикулярно скорости мембраны сообщая этой точке поперечную скорость V*.
